...
def TotalMilkPerWeek():
    conn=get_db_connection()  
    cursor = conn.cursor(dictionary=True)
    query1="SELECT SUM(Quantity) AS total_milk_this_week FROM milkdata WHERE YEARWEEK(Date, 1) = YEARWEEK(CURDATE(), 1);"
    query2="SELECT SUM(Amount) AS total_amount_this_week FROM milkdata WHERE YEARWEEK(Date, 1) = YEARWEEK(CURDATE(), 1);"
    ...
    result = {
        "total_milk_this_week": result1['total_milk_this_week'],
